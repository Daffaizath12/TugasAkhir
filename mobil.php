...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
 if (isset($_POST["action"]) && $_POST["action"] == "delete" && isset($_POST["id_mobil"])) {
  $id_sopir = $_POST["id_mobil"];
     ...
     <div class="col-lg-12 grid-margin stretch-card">
      <div class="card">
       <div class="card-body">
        ...
        <a href="tambah-mobil.php"><button type="button" class="btn btn-success" data-toggle="modal" data-target="#tambahSopirModal">Tambah Mobil</button></a>
          ...
          <tbody>
           ...
           if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
             ...
             echo "<td>" . $row["plat"] . "</td>";
             echo "<td>";
             echo "<button onclick=\"window.location.href='edit-mobil.php?id_mobil=" . $row["id_mobil"] . "'\" class='btn btn-primary'>Edit</button>" . " ";
             echo "<button class='btn btn-danger' data-bs-toggle='modal' data-bs-target='#hapusModal" . $row['id_mobil'] . "'>Hapus</button>";
             // Tombol Konfirmasi Aktifkan
             echo "</td>";
             echo "</tr>";

             // Modal Konfirmasi Hapus
             echo "<div class='modal fade' id='hapusModal" . $row['id_mobil'] . "' tabindex='-1' role='dialog' aria-labelledby='hapusModalLabel' aria-hidden='true'>";
             ...
             echo "Apakah Anda yakin ingin menghapus data Mobil " . $row['nama_mobil'] . "?";
             echo "</div>";
             echo "<div class='modal-footer'>";
             echo "<form method='post' action='mobil.php'>";
             echo "<input type='hidden' name='id_mobil' value='" . $row['id_mobil'] . "'>";
            ...
            echo "<tr><td colspan='5'>Tidak ada data mobil.</td></tr>";
           }
           ?>
          </tbody>
         </table>
